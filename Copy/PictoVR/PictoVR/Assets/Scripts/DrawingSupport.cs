...
public class DrawingSupport : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (touchingLast) 
		{
			if (x - penSize < 0)
				x = penSize;
			if (x + penSize > textureSize)
				x = textureSize - penSize;
			if (y - penSize < 0)
				y = penSize;
			if (y + penSize > textureSize)
				y = textureSize - penSize;
			...
			for (float t = 0.01f; t < 1.00f; t += 0.01f) 
			{
				...
				//Debug.Log("Lerp " + lerpX + "/" + lerpY);
				if (lerpX - penSize < 0)
					lerpX = penSize;
				if (lerpX + penSize > textureSize)
					lerpX = textureSize - penSize;
				if (lerpY - penSize < 0)
					lerpY = penSize;
				if (lerpY + penSize > textureSize)
					lerpY = textureSize - penSize;
				//Debug.Log("New Lerp " + lerpX + "/" + lerpY);
